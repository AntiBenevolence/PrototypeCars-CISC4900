...
public class FWDController : MonoBehaviour
{
    ...
    */
    ...
    void Update()
    {
        ...
        speed = frWheelCollider.rpm*frWheelCollider.radius*2f*Mathf.PI /10f;
...
{
    {
        ...
        {
            gearState = GearStateFWD.Neutral;
        }
        if (gearState == GearStateFWD.Running && clutch > 0)
        {
        ...
        if (isEngineRunning > 0)
        {
            ...
            else
            {
                ...
                rpm = Mathf.Lerp(rpm, Mathf.Max(idleRPM - 100, wheelRPM), Time.deltaTime * 8f);
                torque = (powerCurve.Evaluate(rpm / redLine) * motorPower / rpm) * gearRatios[currentGear] * differentialRatio * 7 * clutch;
            }
        }
        return torque;
    }
    ...
    {
        gearState = GearStateFWD.CheckingChange;
        if (currentGear + gearChange >= 0)
        {
            if (gearChange > 0)
            {
                ...
                {
                    gearState = GearStateFWD.Running;
                    yield break;
                }
            }
            if (gearChange < 0)
            ...
            gearState = GearStateFWD.Changing;
            yield return new WaitForSeconds(changeGearTime);
            currentGear += gearChange;
        }

        if(gearState!=GearStateFWD.Neutral)
        gearState = GearStateFWD.Running;
    }
    ...
    void CheckInput(){
        ...
        if (gearState != GearStateFWD.Changing)
        {
            if (gearState == GearStateFWD.Neutral)
            {
                clutch = 0;
                if (Mathf.Abs( gasInput )> 0) gearState = GearStateFWD.Running;
            }
            else
            {
