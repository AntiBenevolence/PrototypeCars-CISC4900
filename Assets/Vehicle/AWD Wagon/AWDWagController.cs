...
public class AWDWagController : MonoBehaviour
{
...
{
    {
        ...
        {
            gearState = GearStateWag.Neutral;
        }
        if (gearState == GearStateWag.Running && clutch > 0)
        {
    ...
    {
        gearState = GearStateWag.CheckingChange;
        if (currentGear + gearChange >= 0)
        {
            if (gearChange > 0)
            {
                ...
                {
                    gearState = GearStateWag.Running;
                    yield break;
                }
            }
            if (gearChange < 0)
            ...
            gearState = GearStateWag.Changing;
            yield return new WaitForSeconds(changeGearTime);
            currentGear += gearChange;
        }

        if(gearState!=GearStateWag.Neutral)
        gearState = GearStateWag.Running;
    }
    ...
    void CheckInput(){
        ...
        if (gearState != GearStateWag.Changing)
        {
            if (gearState == GearStateWag.Neutral)
            {
                clutch = 0;
                if (Mathf.Abs( gasInput )> 0) gearState = GearStateWag.Running;
            }
            else
            {
